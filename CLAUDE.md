...
- **Bubble arrives after the audio (task 3.14).** In `revealTurnChunk`'s screen-on path, `<MessengerChallengePair>` renders as an empty, fixed-size progress-sweep card (`pending`/`progress` props) while a chunk's own clip plays — closing the hole where the text was visible to read along with during playback (3.13 only hid the *translation*, not the target text itself). Once that first listen finishes, `played` flips permanently true and the target-language zone stops being hover-gated and is just shown — the listening test is over, so hiding it afterward would be friction, not training. The native-language zone stays hover-gated forever, unchanged (3.13's translation-recovery path). `pendingChunkKeys`/`playedChunkKeys`/`activePlayback` in `MessengerChat.tsx` track this, keyed `${messageId}-${index}`; only the screen-on real-turn path ever populates them, so eyes-free and premade chunks fall through to the pre-3.14 legacy rendering untouched. Progress comes from `useAudioPlayer`'s `onProgress` callback (`AudioProgress` in `sharedGameHooks.ts`, wired to the `<audio>` element's `timeupdate`/`loadedmetadata`); an unknown duration (cache miss still resolving) falls back to an indeterminate CSS shimmer (`.progress-shimmer`). Replay (hovering the settled card's audio zone) sweeps its own progress bar without hiding the now-visible text. The card's own width no longer moves on hover: each zone ghost-sizes itself (an always-rendered, invisible copy of its own longest content, stacked via CSS grid) and the card carries `maxWidth: min(60ch, 85%)`.
