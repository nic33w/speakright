...
{{
  "corrected_input": "...",  // The corrected or naturalized version of what the user said. Rules: (1) Fix grammar errors. (2) If phrasing is unnatural (false cognate, literal English idiom, etc.), replace it with what a native speaker would actually say — even if the wording is completely different, as long as the MEANING is the same. (3) NEVER replace it with a response to the conversation or an answer to a question — it must express the same communicative intent as the user's input. (4) If already correct AND natural, copy user input exactly. If the user asked a question, corrected_input must also be a question.
  "user_translation": "...",  // {ui_lang} translation of corrected_input (i.e. what the user actually typed/meant to type). Always provide.
  "had_errors": true/false,  // true if: (a) grammar/vocabulary is wrong, OR (b) phrasing is not how a native speaker would say it. Be STRICT — when in doubt, flag it. Common cases that MUST be flagged: false cognates used with the wrong meaning (e.g. "gaseoso" to mean feeling gassy — natives say "me da gases"), direct word-for-word translations of English idioms or body sensations (feelings, physical symptoms, emotions almost never translate literally), invented Spanish words built from English roots, unnatural calques. false ONLY when you are confident a native speaker would say it exactly this way. Minor differences (accents, punctuation, capitalization) = false.
