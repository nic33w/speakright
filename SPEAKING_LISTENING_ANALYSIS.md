...
**Status (2026-07-19): done**, in 6 commits (stage 0–5). Deviations/decisions: worddrill/story/guessing got their own routers (8 total — worddrill has 7 endpoints, mostly non-check); a pytest harness was added first (27-route mock smoke + prompt goldens, `backend/tests/`) so stages 1–3 are provably no-op; `game_backend.py` remains as a compat shim. Beyond the spec, per user decision: real per-model pricing in `settings.MODEL_PRICING` and **all** LLM calls now record cost (previously messenger-only, at gpt-4o-mini rates regardless of model). §9 restructure landed and is verified end-to-end: static prefix byte-identical across turns (prefix-stability tests enforce it), OpenAI reported `cached_tokens=2304` of 2509 input tokens on a repeat turn. The v2 challenge format needed an end-of-prompt reminder line in the turn instruction after the restructure (the model dropped the format when the v2 block moved earlier). §9 content trims (per-language naturalness examples, `user_translation` drop) deferred by choice. Found along the way: **premade conversations are broken on main** (both premade paths omit the required `input_intent` field → 500; xfail-documented in tests, not fixed — awaiting decision), and the local `.env` runs `OPENAI_MODEL=gpt-4o-mini`, not gpt-4.1-mini as previously documented. See `backend/PROMPTS.md` for the new prompt architecture.
